...
MAX_WORKERS = 99999999999  # Số luồng chạy song song tối đa
...
def process_account_sync(sfile, log_callback):
    ...
    try:
        data = loop.run_until_complete(get_init_data(sfile))
        if not data:
...
async def run(session_files, log_callback=print):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            sfile = futures[future]
            try:
                future.result()
            except Exception as e:
                log_callback(f"[NotBux] Lỗi khi xử lý {sfile}: {e}")
    log_callback("[NotBux] Hoàn tất tất cả tài khoản.")
