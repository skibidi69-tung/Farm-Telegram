...
class NotBuxBot:
    def __init__(self, cfg, log_func=print):
        ...
        # Headers chuẩn, bao gồm Authorization và Content-Type
        self.headers = {
    ...
    def get_balance(self):
        """Lấy số dư từ API /me"""
        try:
            resp = self.session.get('https://notbux.click/api/me', headers=self.headers, timeout=10)
            if resp.status_code == 200:
                data = resp.json()
                return data.get('user', {}).get('balance_coins')
            else:
                self.log(f"[DEBUG] /me status {resp.status_code}")
        except Exception as e:
            self.log(f"[DEBUG] get_balance lỗi: {e}")
        return None

    def claim_daily(self):
        """Claim thưởng hàng ngày (API /earn/checkin)"""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self.log(f"[Daily] HTTP {resp.status_code}")
        except Exception as e:
...
# Hàm chính được gọi từ main_gui.py hoặc script test
async def run(session_files, log_callback=print):
    ...
    for sfile in session_files:
        ...
        if bal is None:
            log_callback(f"[NotBux] Bỏ qua {sfile} do không lấy được balance")
            continue
        bot.run_all()
        time.sleep(5)
